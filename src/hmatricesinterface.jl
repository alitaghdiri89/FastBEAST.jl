...
function interface_to_hmatrix(operator, spaceA, spaceB; rtol=1e-6, splitter_nmax=50)
    ...
    h_matrix = assemble_hmatrix(abstract_matrix, Xclt, Yclt, comp = PartialACA(; rtol=rtol), threads =false)
    return h_matrix 
end

##
@show hmat
@show get_hmatrix_memory_size(hmat)
@show Base.summarysize(hmat)
# @show Base.summarysize(hmat)
##
## Testing
...
Γb = CompScienceMeshes.translate(meshrectangle(1.0, 1.0, 0.2), SVector(3.0, 0.0, 0.0))
spaceA = lagrangecxd0(Γa)
spaceB = lagrangecxd0(Γb)
dim = length(spaceB.pos)

@time A = assemble(op, spaceA, spaceB)
@time hmat = interface_to_hmatrix(op, spaceA, spaceB; rtol=1e-4)
## 
...
##
clt = ClusterTree(spaceX.pos, HMatrices.GeometricSplitter(50))
HMatrices.index_range(clt)