...
@views function LinearAlgebra.mul!(y::AbstractVecOrMat, A::FMMMatrixADL, x::AbstractVector)
    ...
    y .=
        A.op.alpha .* (A.Bt_test * (fmm_res1 + fmm_res2 + fmm_res3)) - A.BtCB * x +
        A.fullmat * x
...
@views function LinearAlgebra.mul!(
    ...
    y .=
        A.op.alpha .* (transpose(A.B_trial) * fmm_res) - transpose(A.BtCB) * x +
        transpose(A.fullmat) * x

    return y
end

@views function LinearAlgebra.mul!(
    y::AbstractVecOrMat, At::LinearMaps.AdjointMap{<:Any,<:FMMMatrixADL}, x::AbstractVector
)
