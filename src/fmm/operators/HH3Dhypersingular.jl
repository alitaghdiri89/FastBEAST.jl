...
@views function LinearAlgebra.mul!(y::AbstractVecOrMat, A::FMMMatrixHS, x::AbstractVector)
    ...
    if A.op.alpha != 0.0
        fmm_res1 =
            A.normals_test[:, 1] .*
            (A.fmm * (A.normals_trial[:, 1] .* (A.B_trial * xfmm)))[:, 1]
        fmm_res2 =
            A.normals_test[:, 2] .*
            (A.fmm * (A.normals_trial[:, 2] .* (A.B_trial * xfmm)))[:, 1]
        fmm_res3 =
            A.normals_test[:, 3] .*
            (A.fmm * (A.normals_trial[:, 3] .* (A.B_trial * xfmm)))[:, 1]
...
@views function LinearAlgebra.mul!(
    y::AbstractVecOrMat, At::LinearMaps.TransposeMap{<:Any,<:FMMMatrixHS}, x::AbstractVector
)
    ...
    if A.op.alpha != 0.0
        fmm_res1 = A.normals_trial[:, 1] .* (A.fmm_t * (A.normals_test[:, 1] .* (xx)))[:, 1]
        fmm_res2 = A.normals_trial[:, 2] .* (A.fmm_t * (A.normals_test[:, 2] .* (xx)))[:, 1]
        fmm_res3 = A.normals_trial[:, 3] .* (A.fmm_t * (A.normals_test[:, 3] .* (xx)))[:, 1]
    ...
    if A.op.beta != 0.0
        fmm_curl1 =
            transpose(A.B1curl_trial) * (A.fmm_t * (transpose(A.B1curl_test) * xfmm))[:, 1]
        fmm_curl2 =
            transpose(A.B2curl_trial) * (A.fmm_t * (transpose(A.B2curl_test) * xfmm))[:, 1]
        fmm_curl3 =
            transpose(A.B3curl_trial) * (A.fmm_t * (transpose(A.B3curl_test) * xfmm))[:, 1]
...
@views function LinearAlgebra.mul!(
    y::AbstractVecOrMat, At::LinearMaps.AdjointMap{<:Any,<:FMMMatrixHS}, x::AbstractVector
)
