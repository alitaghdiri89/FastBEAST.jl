...
@views function LinearAlgebra.mul!(y::AbstractVecOrMat, A::FMMMatrixSL, x::AbstractVector)
    ...
    y .=
        A.op.alpha .* (A.B_test * (A.fmm * (A.B_trial * xfmm))[:, 1]) - A.BtCB * x +
        A.fullmat * x
...
@views function LinearAlgebra.mul!(
    y::AbstractVecOrMat, At::LinearMaps.TransposeMap{<:Any,<:FMMMatrixSL}, x::AbstractVector
)
    ...
    y .=
        A.op.alpha .*
        (transpose(A.B_trial) * (A.fmm_t * (transpose(A.B_test) * xfmm))[:, 1]) -
        transpose(A.BtCB) * x + transpose(A.fullmat) * x

    return y
end

@views function LinearAlgebra.mul!(
    y::AbstractVecOrMat, At::LinearMaps.AdjointMap{<:Any,<:FMMMatrixSL}, x::AbstractVector
)
...
function FMMMatrix(
    ...
    return FMMMatrixSL(
        fmm, fmm_t, op, B, B_test, BtCB, fullmat, size(fullmat)[1], size(fullmat)[2]
    )
end
