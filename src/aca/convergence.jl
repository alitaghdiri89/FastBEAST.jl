...
"""
function initconvergence(M::LazyMatrix{I,K}, convcrit::Standard) where {I,K}
    return convcrit
end

function convergence!(
    tol::F, normUV::F, am::ACAGlobalMemory{I,F,K}, convcrit::Standard, sizeM::I
) where {I,F<:Real,K}
...
function (::RandomSampling{I,F,K})(::Type{K}; factor=F(1.0), nsamples=I(0)) where {I,F,K}
    return RandomSampling(
...
function convergence!(
    tol::F, normUV::F, am::ACAGlobalMemory{I,F,K}, convcrit::RandomSampling{I,F,K}, sizeM::I
) where {I,F<:Real,K}

    # random sampling convergence
    for i in eachindex(convcrit.rest)
        @views convcrit.rest[i] -=
            am.U[convcrit.indices[i, 1], am.npivots] *
            am.V[am.npivots, convcrit.indices[i, 2]]
    end
...
function BalancedCombined(
    ::Type{K}, area::Tuple{Vector{F},Vector{F}}; factor=1.0, nsamples=0
) where {F,K}
...
function (::BalancedCombined{I,F,K})(
    ::Type{K}, area::Tuple{Vector{F},Vector{F}}; factor=1.0, nsamples=0
) where {I,F,K}
    return BalancedCombined(
        area,
...
function convergence!(
    ...
    return (
        sqrt(meanrest * sizeM) <= tol * sqrt(am.normUV²) && normUV <= tol * sqrt(am.normUV²)
    )
end
...
function initconvergence(
    M::LazyMatrix{I,K}, convcrit::BalancedCombined{I,F,K}
) where {I,F<:Real,K}
...
"""
function initconvergence(
    M::LazyMatrix{I,K}, convcrit::Union{RandomSampling{I,F,K},Combined{I,F,K}}
) where {I,F<:Real,K}
    convcrit.nsamples > length(M.τ) * length(M.σ) && println("Conv. oversampled!")
...
function checkconvergence(
    ...
    if (normUV == 0) && (rowpivstrat != FastBEAST.MaxPivoting{I} || am.npivots == 1)
        ...
        return convergence!(tol, normUV, am, convcrit, maxrows * maxcolumns),
        rowpivstrat,
        columnpivstrat
    end
end
