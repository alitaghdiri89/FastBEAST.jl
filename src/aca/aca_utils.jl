...
# Optional Arguments
  ...
  - `maxrank=40`: Maximum rank for the ACA. The ACA stops of more rows/columns are used for
    the approximation.
"""
function allocate_aca_memory(::Type{K}, maxrows::I, maxcolumns::I; maxrank=40) where {I,K}
...
function allocate_aca_memory(
    ...
    if multithreading
        return [
            allocate_aca_memory(K, maxrows, maxcolumns; maxrank=maxrank) for
            i in 1:Threads.nthreads()
        ]
    else
