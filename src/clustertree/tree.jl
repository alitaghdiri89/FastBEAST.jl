...
function BoxTreeOptions(; nmin=50, maxlevel=50)
...
struct KMeansTreeOptions{I}
    ...
    maxlevel::I
    nchildren::I
    KMeansSettings::NminClusterTrees.KMeansSettings{I}
end

function KMeansTreeOptions(;
    nmin=50, maxlevel=50, nchildren=2, KMeansSettings=NminClusterTrees.KMeansSettings()
)
...
function listnearfarinteractions(
    ...
    for chd in ClusterTrees.children(block_tree, block)
        ...
        chd_state = (
            block_tree.test_cluster.nodes[chd[1]], block_tree.trial_cluster.nodes[chd[2]]
        )
...
function value(tree::NminClusterTrees.NminTree{D}, node::I) where {I,D}
    return NminClusterTrees.value(tree, node)
end

function value(tree::NminClusterTrees.NminTree{D}, nodes::Vector{I}) where {I,D}
    values = Int[]
    for node in nodes
        append!(values, NminClusterTrees.value(tree, node))
    end
    return values
end

function levellink(tree::NminClusterTrees.NminTree{D}; node=root(tree), target=1) where {D}
    return Iterators.filter(
        n -> tree.nodes[n].height == target, ClusterTrees.DepthFirstIterator(tree, node)
    )
end

function cluster_link(tree::NminClusterTrees.NminTree{D}) where {D}
    link = Vector{Int}[]
    for level in eachindex(tree.levels)
        nodes = Int[]
        for node in FastBEAST.levellink(tree; target=level - 1)
            push!(nodes, node)
        end
        push!(link, nodes)
    end
...
function child_link(tree::NminClusterTrees.NminTree{D}, node::I) where {I,D}
    ...
    if ClusterTrees.haschildren(tree, node)
        ...
        for childidx in ClusterTrees.children(tree, node)
            push!(
                childrange,
                (childidx, idxcounter:(idxcounter + length(value(tree, childidx)) - 1)),
            )
