...
function ExaFMMOptions(tol; ncrit=50)
    ...
    return error("This function is not yet implemented")
end
"""
    assemble_fmm(
        spoints::Matrix{F},
...
function getfullrankblocks(
    ...
    #=
        @views blkasm = BEAST.blockassembler(
            operator, test_functions, trial_functions; quadstrat=quadstratfbk
        )
        ...
        @views corblkasm = BEAST.blockassembler(
            operator, test_functions, trial_functions; quadstrat=quadstratcbk
        )

        @views function corassembler(Z, tdata, sdata)
            @views store(v, m, n) = (Z[m, n] += v)
            return corblkasm(tdata, sdata, store)
        end
    =#
    ...
    nears, _ = computeinteractions(tree)
    fullrankblocks = assemble(
        operator,
        test_functions,
        trial_functions,
        tree,
    ...
    return fullrankblocks, correctionblocks, nears
end
